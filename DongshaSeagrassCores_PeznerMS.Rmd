...
# Load function (source will vary by computer)
source('~/Documents/SIO/Dongsha 2018/Code/Github/calcDOatsat.R') # load function to calculate DO saturation concentration
...
## Load data (file location will vary by user)
  botdat <- read_excel("~/Documents/SIO/Dongsha 2018/Data/Zenodo/bottledata.xlsx")
  coreloc <- read_excel("~/Documents/SIO/Dongsha 2018/Data/Zenodo/coreGPS.xlsx")
...
# Create data with range of TA and DIC values (every 10 umol/kg) that we're interested in (for contour lines)
...
## Compute linear regressions (p-values will vary slightly depending on number of permutations run, so setting seed for reproducibility)    
library(lmodel2)

  # Linear regression of nTA and nDIC
      set.seed(1000)
      lm_nTAnDIC = botdat %>% 
                    summarize(s = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,3],
                              int = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,2],
                              r2 = lmodel2(nTA~nDIC, nperm = 1000)$rsquare,
                              pval = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,5])

     # Linear regression of nTA and nDIC by survey
      set.seed(1000)
      lm_nTAnDIC_survey = botdat %>% 
                    dplyr::group_by(Survey) %>% 
                    dplyr::summarize(s = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,3],
                              int = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,2],
                              r2 = lmodel2(nTA~nDIC, nperm = 1000)$rsquare,
                              pval = lmodel2(nTA~nDIC, nperm = 1000)$regression.results[1,5])

      
  # Linear regression of nDO and nDIC_NCP
      set.seed(1000)
      lm_DO_nDIC_NCP = botdat %>% 
                    summarize(s = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,3],
                              int = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,2],
                              r2 = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$rsquare,
                              pval = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,5])
      
      # Linear regression of nDO and nDIC_NCP by survey
      set.seed(1000)
      lm_DO_nDIC_NCP_survey = botdat %>% 
                                dplyr::group_by(Survey) %>% 
                                dplyr::summarize(s = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,3],
                                          int = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,2],
                                          r2 = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$rsquare,
                                          pval = lmodel2(DO_umolkg~nDIC_NCP, nperm = 1000)$regression.results[1,5])
...
## PLOT nTA vs nDIC
  ...
  # Plot nTA vs nDIC -- colored by survey (shape = location)    
    pp  = ggplot()+
            ...
            geom_abline(data = lm_nTAnDIC, mapping = aes(slope = s, intercept = int), color="black",linetype = "dashed")+
  ...
  # DO vs nDIC_NCP (color by survey)
    pp2 = ggplot()+
            ...
            metR::geom_contour2(data = D3, aes(x=nDIC_NCP, y=DO ,z=npH, label =  round(..level..,3)), 
                                color = "grey", alpha = 0.9, label_size = 9)+
            # Plot linear regression line
            geom_abline(data = lm_DO_nDIC_NCP, mapping = aes(slope = s, intercept = int), color="black",linetype = "dashed" )+
...
## Load data --
  DSHA_IDRO2 = read.csv('~/Documents/SIO/Dongsha 2018/Data/Zenodo/idronaut.csv')
...
## SPATIAL SURVEY 
  ...
  # Conduct linear regression of pH vs DO
    set.seed(1000)
    lm_pH_DO_ss = botdat %>% 
                  summarize(s = lmodel2(pH_TA_DIC~DO_umolkg, nperm = 1000)$regression.results[1,3],
                            int = lmodel2(pH_TA_DIC~DO_umolkg, nperm = 1000)$regression.results[1,2],
                            r2 = lmodel2(pH_TA_DIC~DO_umolkg, nperm = 1000)$rsquare,
                            pval = lmodel2(pH_TA_DIC~DO_umolkg, nperm = 1000)$regression.results[1,5])
...
## IDRONAUT
  ...
  # Conduct linear regression of pH vs DO
    ...
    lm_pH_DO_idro = idro_sg %>% 
                      summarize(s = lmodel2(pH_T~DO_umolkg, nperm = 1000)$regression.results[1,3],
                                int = lmodel2(pH_T~DO_umolkg, nperm = 1000)$regression.results[1,2],
                                r2 = lmodel2(pH_T~DO_umolkg, nperm = 1000)$rsquare,
                                pval = lmodel2(pH_T~DO_umolkg, nperm = 1000)$regression.results[1,5])    
...
## Load data and adjust it
  # Load file
    cores = read_excel("~/Documents/SIO/Dongsha 2018/Data/Zenodo/coreXDS.xlsx")
...
## Subset cores into "cores_6y" which only contains data for years 2012-2017 (the 6 years they all have in common)
cores_6y = cores %>% 
                filter(year > 2011)
...
lmodel2_results = cores_6y %>%
                  dplyr::group_by(location) %>% 
                  dplyr::summarise(s_evd = lmodel2(extension~density, nperm = 1000)$regression.results[1,3],
                          int_evd = lmodel2(extension~density, nperm = 1000)$regression.results[1,2],
                          r2_evd = lmodel2(extension~density, nperm = 1000)$rsquare,
                          pval_evd = lmodel2(extension~density, nperm = 1000)$regression.results[1,5],
                          
                          s_cve = lmodel2(calcification~extension, nperm = 1000)$regression.results[1,3],
                          int_cve = lmodel2(calcification~extension, nperm = 1000)$regression.results[1,2],
                          r2_cve = lmodel2(calcification~extension, nperm = 1000)$rsquare,
                          pval_cve = lmodel2(calcification~extension, nperm = 1000)$regression.results[1,5],
                          
                          s_dvc = lmodel2(density~calcification, nperm = 1000)$regression.results[1,3],
                          int_dvc = lmodel2(density~calcification, nperm = 1000)$regression.results[1,2],
                          r2_dvc = lmodel2(density~calcification, nperm = 1000)$rsquare,
                          pval_dvc = lmodel2(density~calcification, nperm = 1000)$regression.results[1,5])
...
## Calculate mean calcification, extension, density rates for each location across all years with 95% CI
  cores_meansCI = cores %>% dplyr::group_by(location, year) %>% 
...
## Add means and CIs to cores df by year and location for plotting
  cores_all = merge(cores,cores_meansCI,by= c("location","year"))

## Plots with means + CI on top

  # Extension
  s3a =  ggplot() +
    geom_line(data = cores_all, aes(x = year, y = extension, group = coreID, color = location), alpha = 0.3, size = 1) +
    geom_ribbon(data = cores_all, aes(ymin = ext_95CI_lower, ymax = ext_95CI_upper, x = year, fill = location), alpha = 0.3)+
    geom_line(data = cores_all, aes(y = ext_mean, x = year, color = location), size = 1.3)+
  ...
  # Density
  s3b =   ggplot() +
    geom_line(data = cores_all, aes(x = year, y = density, group = coreID, color = location), alpha = 0.3, size = 1) +
    geom_ribbon(data = cores_all, aes(ymin = dens_95CI_lower, ymax = dens_95CI_upper, x = year, fill = location), alpha = 0.3) +
    geom_line(data = cores_all, aes(y = dens_mean, x = year, color = location), size = 1.3)+
    facet_wrap(~ location, ncol=4) +
    scale_color_manual(values = core_pal) + scale_fill_manual(values = core_pal) +
    scale_x_continuous(breaks = seq(1995,2015,5)) +
    theme_classic() + theme(legend.position = "none",
  ...
  # Calcification
  s3c =  ggplot() +
    geom_line(data = cores_all, aes(x = year, y = calcification, group = coreID, color = location), alpha = 0.3, size = 1) +
    geom_ribbon(data = cores_all, aes(ymin = calc_95CI_lower, ymax = calc_95CI_upper, x = year, fill = location), alpha = 0.3) +
    geom_line(data = cores_all, aes(y = calc_mean, x = year, color = location), size = 1.3)+
    facet_wrap(~ location, ncol=4) +
    scale_color_manual(values = core_pal) + scale_fill_manual(values = core_pal) +
    scale_x_continuous(breaks = seq(1995,2015,5)) +
    theme_classic() + theme(legend.position = "none",
...
```{r TABLE S1 -- DESCRIPTIVE STATS FROM SPATIAL SURVEYS}
# Early morning (S3)
  s3_stats = s3 %>% summarize(survey = "S3",
                     minT = min(Temp),
                     maxT = max(Temp),
    ...
    # Late afternoon (S1)
    s1_stats = s1 %>% summarize(survey = "S1",
                     minT = min(Temp),
                     maxT = max(Temp),
                     rangeT = max(Temp)-min(Temp),
                     meanT = mean(Temp),
    ...
    # Mid-morning (S2)
    s2_stats = s2 %>% summarize(survey = "S2",
                     minT = min(Temp),
                     maxT = max(Temp),
                     rangeT = max(Temp)-min(Temp),
                     meanT = mean(Temp),
    ...
    # Mid-day (S4)
    s4_stats = s4 %>% summarize(survey = "S4",
                     minT = min(Temp),
                     maxT = max(Temp),
                     rangeT = max(Temp)-min(Temp),
                     meanT = mean(Temp),
...
#### Means across all years and cores (Results section)
    cores_means = cores %>% 
                       dplyr::summarize(mean_ext = mean(extension, na.rm = T),
                                        sd_ext = sd(extension, na.rm = T),
                                        mean_dens = mean(density, na.rm = T),
